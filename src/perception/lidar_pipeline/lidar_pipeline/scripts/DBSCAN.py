...
def get_objects_np(object_points: np.array) -> List[List]:
    EPSILON = 0.4  # Neighbourhood Scan Size
    MIN_POINTS = 3 # Number of points required to form a neighbourhood
    ...
    for i in range(len(clusters)):
        ...
        #assume the cone is sphrical, calculate the average size of the spread
        avgdist = np.sum(np.sqrt(np.square(np.array(x_cluster)-x_mean) + np.square(np.array(y_cluster)-y_mean) + np.square(np.array(z_cluster)-z_mean)))/len(x_cluster)
        print(f"Aver dist: {avgdist} Num points: {len(x_cluster)}")
        clusters[i] = [x_mean, y_mean, z_mean]
